...
<div id="contenedor-Preguntas">
    ...
    <div class="Menus-create">
        <div class="col-md-12" id="btn_next" style="display:none;">
            ...
            <div class="form-group" style="text-align: end !important;">
                <?= Html::submitButton(Yii::t('app', $textoBotton . ' <em class="fas fa-arrow-right"></em>'), ['class' => 'btn btn-primary text-center', 'onclick' => $funcionEnvioRespuestas . '("memoriaauditiva","' . $accion . '","' . $modelSeccion->secTipoRespuesta . '","solo")']) ?>
            </div>
        </div>
        <div class="name-tag">
            <h1><?= $this->title ?></h1>
        ...
        <div class="row" id='preguntaAdicional' style="display:none;">
            <audio id="audioPreguntaAdicional" preload="metadata">
                <source src="https://docs.google.com/uc?export=open&id=<?= $modelSeccion->seccAudioPreguntaAdicional  ?>" type="audio/mp3">
            </audio>
            <div class="col-md-12">
                <button class="btnPersonalizado" onclick="reproducirAudioCargado('audioPreguntaAdicional','iconoButtonPreguntaAdicional','fas fa-volume-off','fas fa-volume-up')"><em id="iconoButtonPreguntaAdicional" class="fas fa-volume-off"></em></button>
                <label id="preguntaAdicionalID" name="preguntaAdicionalID">
                    <?= $modelSeccion->seccPreguntaAdicional ?>
                </label>
            </div>
        </div>
        ...
        <?= IntentaloNuevamente::widget([
            'funcionRepetir' => 'repetirImagenes6',
    ...
    <button class="btn btn-primary text-center" id="btmContinuar" onclick="siguientePregunta4(<?= count($modelRespuestas) ?>,'<?= $modelSeccion->secTipoRespuesta ?>')"> Continuar <em class="fas fa-arrow-right"></em></button>
</div>
